...
async function createAsset(params: {
  ...
  const item: CatalogItem = {
    ...
    license: params.licenseId ? licenseLabels[params.licenseId] : cleanText(params.license, licenseLabels['cc0'], 80),
    licenseId: params.licenseId || 'cc0',
  ...
  async init() {
    ...
    this.server.tool(
      ...
      'Create a hosted SVG illustration, icon, pattern, background, or UI asset under the authenticated creator account.',
      {
        ...
        license: z.enum(licenseIds).optional().describe('License id (default cc0 — all FDS assets are public-domain dedicated)'),
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname === '/health') {
      ...
      return Response.json({
        ...
        tools: mcpDiscoveryTools.length,
      });
    }
...
function wrongEndpoint(): Response {
  return new Response(
    JSON.stringify({
      ...
      error: {
        code: -32000,
        message: 'Method Not Allowed — the MCP endpoint is https://mcp.freedesignstore.online/mcp',
      },
    }),
